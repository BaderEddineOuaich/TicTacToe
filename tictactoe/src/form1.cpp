...
static const size WINDOW_SIZE = { 640, 480 };
static const size BUTTON_SIZE = { 80, 80 };

static const std::array<point, 9> BUTTON_LOCATIONS =
{
  point(100 + BUTTON_SIZE.width(),     100),
  point(100 + BUTTON_SIZE.width() * 2, 100),
  point(100 + BUTTON_SIZE.width() * 3, 100),

  point(100 + BUTTON_SIZE.width(),     100 + BUTTON_SIZE.height()),
  point(100 + BUTTON_SIZE.width() * 2, 100 + BUTTON_SIZE.height()),
  point(100 + BUTTON_SIZE.width() * 3, 100 + BUTTON_SIZE.height()),

  point(100 + BUTTON_SIZE.width(),     100 + BUTTON_SIZE.height() * 2),
  point(100 + BUTTON_SIZE.width() * 2, 100 + BUTTON_SIZE.height() * 2),
  point(100 + BUTTON_SIZE.width() * 3, 100 + BUTTON_SIZE.height() * 2)
};
...
form1::form1() {
  ...
  {
    m_buttons[i].reset(new button());
    m_buttons[i]->flat_style(flat_style::popup);
    m_buttons[i]->parent(*this)
      .text(xtd::ustring::empty_string())
      .font(xtd::drawing::font(font_family::generic_sans_serif(), 24, font_style::bold))
  ...
  // Labels
  m_player_x_score_label.reset(new label());
  m_player_x_score_label->parent(*this)
    .text(xtd::ustring::format("Player X: {}", m_player_x_score))
    .auto_size(true)
    .location({ 5, 5 })
    .font(xtd::drawing::font(font_family::generic_sans_serif(), 12))
    .fore_color(color::orange);
  
  m_player_o_score_label.reset(new label());
...
bool form1::there_is_a_winner() {
  ...
  // |----|----|----|
  // | C1 | C2 | C3 |
  const button& A1 = *m_buttons[0];
  const button& A2 = *m_buttons[1];
  const button& A3 = *m_buttons[2];
  
  const button& B1 = *m_buttons[3];
  const button& B2 = *m_buttons[4];
  const button& B3 = *m_buttons[5];
  
  const button& C1 = *m_buttons[6];
  const button& C2 = *m_buttons[7];
  const button& C3 = *m_buttons[8];
...
{
  button& button = static_cast<xtd::forms::button&>(sender);
  ...
  if (there_is_a_winner())
  {
    ...
    auto res = message_box::show(*this, xtd::ustring::format("Player {} is the WINNER!\nDo you want to play again ?", m_XTurn ? 'X' : 'O'), "Winner!", message_box_buttons::yes_no);
  ...
  {
    auto res = message_box::show(*this, "No Winner!", "Tie!", message_box_buttons::yes_no);
    if (res == dialog_result::yes) {
      reset();
    }
    else {
